...
#Take note of response set size, no need for explanatory data that does is not within response bounds
...
# merge rel and abs into one dataset
fw <- merge(fw_abs, fw_rel, by = c("row.names", 'emission'))
tw <- merge(tw_abs, tw_rel, by = c("row.names", 'emission'))

#delete all columns that have CO2E has they are defined as fractions of the response data
fw$CO2E_GF_KT...Value <- NULL
fw$CO2E_KD_GD...Value <- NULL
fw$CO2E_LF_KT...Value <- NULL
fw$CO2E_SF_KT...Value <- NULL
fw$CO2_BLDG_MT...Value <- NULL
fw$CO2_ETOT_MT...Value <- NULL
fw$ATM_CO2E_KT...Value <- NULL
fw$ATM_CO2E_PC...Value <- NULL
fw$CO2_MANF_MT...Value <- NULL
fw$CO2_OTHX_MT...Value <- NULL
fw$CO2_TRAN_MT...Value <- NULL
...
tw$CO2E_GF_KT...Value <- NULL
tw$CO2E_KD_GD...Value <- NULL
tw$CO2E_LF_KT...Value <- NULL
tw$CO2E_SF_KT...Value <- NULL
tw$CO2_BLDG_MT...Value <- NULL
tw$CO2_ETOT_MT...Value <- NULL
tw$ATM_CO2E_KT...Value <- NULL
tw$ATM_CO2E_PC...Value <- NULL
tw$CO2_MANF_MT...Value <- NULL
tw$CO2_OTHX_MT...Value <- NULL
tw$CO2_TRAN_MT...Value <- NULL
...
# delete columns that contain duplicate information from fw_abs and tw_abs
fw$AGR_TOTL_CN...Value <- NULL
fw$AGR_TOTL_KD...Value <- NULL
fw$AGR_TOTL_KN...Value <- NULL
tw$AGR_TOTL_CN...Value <- NULL
...
# reset the row names to the dates so that correlations can be calculated
row.names(fw) <- fw$Row.names
fw$Row.names <- NULL

row.names(tw) <- tw$Row.names
tw$Row.names <- NULL

# try the correlation matrices
cor_fw <- cor(fw)
cor_tw <- cor(tw)
...
fw_training_length <- length(fw$emission) - 10

tw_training_length <- length(tw$emission) - 10
...
attach(fw)
fw_fit <- lm(formula = emission ~ ., data = fw, na.action = na.omit) # all variable fit
summary(fw_fit)
detach(fw)
...
print(c(tw_abs_fit_cvrmse, tw_abs_fit7_cvrmse))
